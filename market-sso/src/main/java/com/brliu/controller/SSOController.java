...
public class SSOController {
    ...
    private boolean verifyUserTicket(String userTicket) {
        ...
        // 1. 验证CAS门票是否有效
        String userId = redisClient.get(REDIS_USER_TICKET + ":" + userTicket);
        if (StringUtils.isBlank(userId)) {
            return false;
        }

        // 2. 验证门票对应的user会话是否存在
        String userRedis = redisClient.get(REDIS_USER_TOKEN + ":" + userId);
    ...
    public String doLogin(String username,
        ...
        redisClient.set(REDIS_USER_TOKEN + ":" + userResult.getId(),
        ...
        redisClient.set(REDIS_USER_TICKET + ":" + userTicket, userResult.getId());
...
//        return "login";
    ...
    public ResponseMessage verifyTmpTicket(String tmpTicket,
        ...
        // 使用完毕后，需要销毁临时票据
        String tmpTicketValue = redisClient.get(REDIS_TMP_TICKET + ":" + tmpTicket);
        if (StringUtils.isBlank(tmpTicketValue)) {
            return ResponseMessage.error("用户票据异常");
        }
        ...
        if (!tmpTicketValue.equals(MD5Utils.getMD5Str(tmpTicket))) {
            ...
            // 销毁临时票据
            redisClient.delete(REDIS_TMP_TICKET + ":" + tmpTicket);
        }
        ...
        // 验证成功，返回OK，携带用户会话
        return ResponseMessage.success(JsonUtils.jsonToPojo(userRedis, UsersVO.class));
    }
    ...
    public ResponseMessage logout(String userId,
        ...
        redisClient.delete(REDIS_USER_TICKET + ":" + userTicket);

        // 2. 清除用户全局会话（分布式会话）
        redisClient.delete(REDIS_USER_TOKEN + ":" + userId);
    ...
    private String createTmpTicket() {
        ...
        try {
            redisClient.set(REDIS_TMP_TICKET + ":" + tmpTicket,
                    MD5Utils.getMD5Str(tmpTicket), 600, TimeUnit.SECONDS);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return tmpTicket;
